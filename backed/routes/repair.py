...
def add_repair_bill():
    ...
    # 验证必填字段
    required_fields = ['owner_id', 'house_id', 'content', 'worker_id']
    ...
    try:
        ...
        # 验证维修工是否存在
        worker = RepairWorker.query.filter_by(id=data['worker_id']).first()
        if not worker:
            return jsonify({
                'code': 400,
                'message': '维修工不存在'
        ...
        bill_dict['worker_name'] = worker.name  # 返回维修工姓名
...
def complete_repair(id):
    ...
    try:
        ...
        if bill.owner:
            bill_dict['owner_name'] = bill.owner.name
            bill_dict['owner_id'] = bill.owner.number
        if bill.house:
            bill_dict['house_number'] = bill.house.number
            bill_dict['house_id'] = bill.house.number
        if bill.worker:
            bill_dict['worker_name'] = bill.worker.name
